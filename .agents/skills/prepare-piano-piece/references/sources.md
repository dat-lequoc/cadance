...
Search the web using the title/catalogue/movement with MIDI, PDF, MusicXML or LilyPond. Inspect the actual publisher/engraver source pages and download targets. When no score was supplied, a pair generated from the same engraving source is preferable to independent files with similar names: voice assignments, repeats and bar numbering are easier to verify. MusicXML/LilyPond source is useful for parts even when the user only needs MIDI and PDF in the app. Prefer vector PDFs when choosing an edition; a supplied scan remains the target and can support manually reviewed bar alignment.
...
If the sources disagree, locate the first divergence and record a printed-bar-to-MIDI-bar correspondence, including pickups, repeats, cadenzas and endings. A count difference alone does not establish missing music or justify replacing a supplied score. First seek MIDI or editable notation matching the target PDF. When permitted source notation exists, generating compatible MIDI is valid; verify it against the target PDF. A re-engraved PDF is an alternative edition requiring agreement when it would replace a supplied target. Label derived files and record the renderer/version/command. Keep original downloads unchanged. When generating voice-separated MIDI for hand assignments, match it back to the chosen original event stream and preserve original durations/velocities unless changing them is part of the task.
