...
The app’s ordinary JSON file picker accepts the simplified legacy format, not normalized Song JSON. Bundle normalized data under `src/core/SLUG.json` for automatic discovery, or import a prepared MIDI whose track names/parts reproduce the same fingerprint. Exact original MIDI bytes also recover bundled reviewed assignments through the frontend importer. Do not tell users to load an internal normalized JSON through that picker.
...
Set `version: 1`, unique plan `id`, `title`, `song: { title, fingerprint }`, `repetitions` (1–100), `counting` (`total` or `consecutive`), and `quests` in suggested practice order (1–1000). Every quest is unlocked from the start; section labels provide dividers and selector groups, not access gates. Every quest needs unique `id`, `title`, `section`, `instruction`, inclusive `fromBar`/`throughBar`, `hand`, `focus`, `mode` (`wait`/`rhythm`), and `speed` (0.25–1.5).
