...
for (const [start, end, section] of sections) for (let from = start; from <= end;) {
  ...
  for (const hand of ["right", "left", "both"] as const) {
    const q = { ...base, id: `bars-${from}-${through}-${hand}`, title: `Bars ${from}–${through} · ${hand} hand${hand === "both" ? "s" : ""}`, hand };
  ...
  const a = bars[from - 1], b = bars[through - 1], notes = song.notes.filter((n) => n.tick >= a.tick && n.tick < b.endTick);
  table += `| ${from}–${through} | ${a.start.toFixed(3)}–${b.end.toFixed(3)} | ${notes.filter((n) => n.hand === "right").length} / ${notes.filter((n) => n.hand === "left").length} |\n`;
  from = through + 1;
}
